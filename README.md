...
面板可以收起。收起后左侧会保留一个小提示，点击“打开拉黑面板”可以打开。
...
**拉黑和导流识别**需要请求 X。
导流号要查询账号主页，拉黑要调用 X 的拉黑接口。这些都受平台规则限制：
...
- **拉黑新号**只能作为高误伤选项，不适合默认开启。它支持按天数或按具体日期筛选，日期默认是一个月之前的今天。它的实际收益有限，因为注册时间查询很慢，热门回复区里你可能已经看完了，后台还没查完。
...
- **拉黑后页面不会总是立刻消失**：X 不一定会立即重新渲染页面。清理完成后刷新页面，已拉黑账号通常就会消失。
